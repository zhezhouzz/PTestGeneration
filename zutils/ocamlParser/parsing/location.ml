      ...
      | [ (line, line_nb, line_start_cnum) ] ->
          ...
          String.iteri
            (fun i c ->
              ...
              else if pos < rightmost.pos_cnum then
  ...
  let message =
    Printf.sprintf
      "OCaml's lib directory layout changed in 5.0. The %s subdirectory has \
       been automatically added to the search path, but you should add -I +%s \
       to the command-line to silence this alert (e.g. by adding %s to the \
       list of libraries in your dune file, or adding use_%s to your _tags \
       file for ocamlbuild, or using -package %s for ocamlfind)."
      lib lib lib lib lib
  in
  let alert =
    {
