...
(** HACK: make it to be consistent with ppxlib *)
...
let letter_alert tokens =
  ...
  | example :: _ ->
      ...
      let spelling_hint ppf =
        let max_seq_len =
          List.fold_left
            (fun l x -> Zint.max l (List.length x))
...
let parse_opt error active errflag s =
  ...
  let eval = function
    ...
    | Num (n1, n2, modifier) ->
        for n = n1 to Zint.min n2 last_warning_number do
          action modifier n
        done
  in
...
let pp_since out release_info =
  Printf.fprintf out " (since %d.%0*d)" release_info.Zsys.major
